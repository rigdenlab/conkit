...
.. code-block:: bash
   ...
   $> conkit-convert toxd/toxd.a3m a3m toxd/toxd.mfa fasta

The call above converts the ``toxd.a3m`` file, which is in ``a3m`` format, to the ``toxd.mfa`` file in ``fasta`` format.
